...
model.preterm.reduced <- glm(as.factor(Preg.ended...37.wk) ~ . - 
                               cat_birthweight - Drug.Add - Use.Alc , 
                             data = opt_logistic, 
          family = binomial, na.action = na.exclude)
...
```
We have two models above. The bigger model has these variables **Clinic**, **Age**, **Ethnicity**, **Education**, **Number of previous pregnancies**, **Diabetes**, **Hypertension**, **Alcohol use**, **Number of qualifying teeth**, **Percentage of sites bleeding on probing at baseline**, **Whole-mouth average pocket depth at baseline (mm)**, **Self-reported participant history of drug addiction** and **Self-reported participants history of alcohol use**. The reduced model does not have drug addiction and alcohol use variables. 
...
compareCoefs(model.preterm,model.preterm.reduced)
```


When looking at the bigger model, the 
